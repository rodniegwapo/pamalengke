    ...
    <div class="flex flex-wrap lg:py-8 lg:pt-20 pt-10">
      <div class="sticky lg:w-1/4 md:w-1/4 md:mt-4 w-full h-auto">
        <SearchFilter
          ...
          class="lg:sticky lg:block md:block block py-4 lg:px-2 md:px-0 px-4"
          @filterByBrand="filterByBrand"
          @filterByPrice="filterByPrice"
        />
        <div class="lg:hidden md:hidden block flex w-full mt-2 ">
      ...
      <div class="flex flex-wrap content-start lg:w-3/4 md:w-3/4 w-full h-auto pt-8">
        <div class="lg:block md:block hidden flex w-full mt-2 ">
          <BevHeader :category="filter.name" :class-products="filter.class" />
        </div>
        <div class="flex flex-wrap content-start w-full lg:mt-10 mt-2">
          <div class="w-full">
...
export default {
  ...
  async mounted () {
    await this.$nextTick(async () => {
      this.$nuxt.$loading.start()
      const result = await this.$axios.get('api/public/category-by-slug/' + this.$route.params.slug, {
        params: {
